<template>
  <div class="space-y-4">
    ...
    <div class="flex justify-end">
      <button 
        @click="exportToExcel"
        class="text-sm text-green-600 hover:text-green-800 px-4 py-2 border border-green-600 rounded-md hover:bg-green-50 flex items-center gap-2 transition-colors"
      >
    ...
    <div v-if="results.length > 0" class="bg-white shadow rounded-lg divide-y">
      <!-- Main Result -->
      <div v-for="(result, index) in results" :key="index" class="bg-white shadow overflow-hidden sm:rounded-lg">
        <div class="px-4 py-5 sm:px-6 border-b border-gray-200">
          <h3 class="text-lg leading-6 font-medium text-gray-900">Resultaat</h3>
          <p class="mt-1 max-w-2xl text-sm text-gray-500">BAG informatie voor dit adres</p>
        </div>
        ...
        <div class="border-b border-gray-200">
          <dl>
            <div class="bg-gray-50 px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
              ...
              <dd class="mt-1 text-sm text-gray-900 sm:col-span-2">{{ result.address || 'Onbekend' }}</dd>
            </div>
            <div class="bg-white px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
              <dt class="text-sm font-medium text-gray-500">Gebruiksdoel</dt>
              <dd class="mt-1 text-sm text-gray-900 sm:col-span-2">{{ result.gebruiksdoel || 'Onbekend' }}</dd>
            </div>
            <div class="bg-gray-50 px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
              <dt class="text-sm font-medium text-gray-500">Bouwjaar</dt>
              <dd class="mt-1 text-sm text-gray-900 sm:col-span-2">{{ result.bouwjaar || 'Onbekend' }}</dd>
            </div>
            <div class="bg-white px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
              <dt class="text-sm font-medium text-gray-500">Status</dt>
              <dd class="mt-1 text-sm text-gray-900 sm:col-span-2">{{ result.status || 'Onbekend' }}</dd>
            </div>
            <div class="bg-gray-50 px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
              <dt class="text-sm font-medium text-gray-500">Oppervlakte</dt>
              <dd class="mt-1 text-sm text-gray-900 sm:col-span-2">{{ result.oppervlakte ? `${result.oppervlakte} m²` : 'Onbekend' }}</dd>
            </div>
            <div class="bg-white px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
              <dt class="text-sm font-medium text-gray-500">Gemeente</dt>
              <dd class="mt-1 text-sm text-gray-900 sm:col-span-2">{{ result.gemeente || 'Onbekend' }}</dd>
            </div>
          </dl>
        </div>
        ...
        <!-- Technical Details -->
        <div class="px-4 py-5 sm:px-6 border-b border-gray-200">
          <h4 class="text-base font-medium text-gray-900">Technische Details</h4>
        </div>
        <div class="border-b border-gray-200">
          <dl>
            <div class="bg-gray-50 px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
              <dt class="text-sm font-medium text-gray-500">Nummeraanduiding ID</dt>
              <dd class="mt-1 text-sm text-gray-900 sm:col-span-2">{{ result.nummeraanduidingId || 'Onbekend' }}</dd>
            </div>
            <div class="bg-white px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
              <dt class="text-sm font-medium text-gray-500">Verblijfsobject ID</dt>
              <dd class="mt-1 text-sm text-gray-900 sm:col-span-2">{{ result.verblijfsobjectId || 'Onbekend' }}</dd>
            </div>
            <div class="bg-gray-50 px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
              <dt class="text-sm font-medium text-gray-500">Pand ID</dt>
              <dd class="mt-1 text-sm text-gray-900 sm:col-span-2">{{ result.pandId || 'Onbekend' }}</dd>
            </div>
          </dl>
        </div>

        <!-- Related Addresses -->
        <div v-if="result.relatedAddresses && result.relatedAddresses.length > 0">
          <div class="px-4 py-5 sm:px-6 border-b border-gray-200">
            <h4 class="text-base font-medium text-gray-900">Gerelateerde Adressen in dit Pand</h4>
            <p class="mt-1 max-w-2xl text-sm text-gray-500">Andere adressen die in hetzelfde gebouw zijn geregistreerd</p>
          </div>
            ...
            <table class="min-w-full divide-y divide-gray-200">
              <thead class="bg-gray-50">
                <tr>
                  ...
                  <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Nummeraanduiding ID</th>
                  <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Verblijfsobject ID</th>
                  <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Gemeente</th>
                </tr>
              </thead>
              <tbody class="bg-white divide-y divide-gray-200">
                <tr v-for="(related, rIndex) in result.relatedAddresses" :key="rIndex">
                  ...
                  <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ related.nummeraanduidingId || 'Onbekend' }}</td>
                  <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ related.verblijfsobjectId || 'Onbekend' }}</td>
                  <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ related.gemeente || 'Onbekend' }}</td>
                </tr>
              </tbody>
            </table>
          </div>
      ...
      <div v-if="debugInfo" class="pt-12 pb-16 flex flex-col items-center">
        ...
        <div v-show="showDebug" class="mt-4 p-4 bg-gray-100 rounded-md text-sm w-full">
...
import { ChevronDownIcon, ChevronUpIcon, DocumentArrowDownIcon } from '@heroicons/vue/24/outline'
...
const exportToExcel = () => {
  ...
  ws['!cols'] = Object.keys(colWidths).map(key => ({ wch: Math.min(colWidths[key] + 2, 50) }))
